<?php echo modal_header($page->title . ' <small>' . lang('documents') . ' ' . (count($documents) ? '<strong id="count-items">(' . count($documents) . ')</strong>' : '') . '</small>') ?>
<?php $imgs = get_icons() ?>
<div class="multimedia">
	...
	<?php endif ?>
	<?php echo form_close(); ?>
	<form id="form-documents" onsubmit="return validate_data(this, '<?php echo base_url() ?>cms/page/save_descriptions/', 'btn-save-files', null, save_textareas)">
				...
				<div class="col-md-3">
					<div class="thumbnail" data-toggle="tooltip" data-placement="bottom" title="<?php echo $document->filename ?>">
						...
						<div class="caption" id="buttons-document">
							<a target="_blank" href="http://docs.google.com/viewer?url=<?php echo urlencode(base_url() . 'assets/files/pages/documents/'. $document->filename) ?>" type="button" class="preview btn btn-default btn-xs" title="Vista previa" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-eye-open"></span></a>
							<a href="<?php echo base_url() . 'assets/files/pages/documents/'. $document->filename ?>" class="btn btn-default btn-xs" title="Descargar" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-download"></span></a>
...
<script type="text/javascript">
    ...
    $('#form-document').on('submit', function (event) {
        ...
        $.ajaxFileUpload({
			url           : _base_url + 'cms/page/upload_documents/<?php echo $id_page ?>',
			secureuri     : false,
			...
			success  : function (response) {
				...
				if (errors.length) {
           		...
           		$.get(_base_url + 'cms/page/get_files/<?php echo $id_page ?>/DOCUMENT', function(data) {
