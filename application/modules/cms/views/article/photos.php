<?php echo modal_header($article->title . ' <small>' . lang('photos') . ' ' . (count($photos) ? '<strong id="count-items">(' . count($photos) . ')</strong>' : '') . '</small>') ?>
<div class="multimedia">
	...
	<?php endif ?>
	<?php echo form_close(); ?>
	<form id="form-photos" onsubmit="return validate_data(this, '<?php echo base_url() ?>cms/article/save_descriptions/', 'btn-save-files', null, save_textareas)">
					...
					<div class="thumbnail">
						...
						<div class="caption" id="buttons-photo">
							<div class="bg-buttons">
								<button type="button" class="preview btn btn-default btn-xs" data-role="<?php echo base_url() . 'assets/files/articles/photos/'. $photo->filename ?>" title="Vista previa" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-eye-open"></span></button>
								<a href="<?php echo base_url() . 'cms/article/download/' . $photo->filename ?>" class="btn btn-default btn-xs" title="Descargar" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-download"></span></a>
...
<script type="text/javascript">
    ...
    $('#form-photo').on('submit', function (event) {
        ...
        $.ajaxFileUpload({
			url           : _base_url + 'cms/article/upload_photos/<?php echo $id_article ?>',
			secureuri     : false,
			fileElementId : 'photo',
			dataType      : 'json',
			data          : {},
			success  : function (response) {
				...
				if (errors.length) {
           		...
           		$.get(_base_url + 'cms/article/get_files/<?php echo $id_article ?>/PHOTO', function(data) {
    ...
    $buttons_portada.on('click', function() {
		...
		$.get(_base_url + 'cms/article/set_primary_file/<?php echo $id_article ?>/' + $this.data('role') + '/PHOTO', function(data) {
			hide_loading();
