<?php echo modal_header($article->title . ' <small>' . lang('videos') . ' ' . (count($videos) ? '<strong id="count-items">(' . count($videos) . ')</strong>' : '') . '</small>') ?>
<div class="multimedia">
	...
	<?php endif ?>
	<form id="form-videos" onsubmit="return validate_data(this, '<?php echo base_url() ?>cms/article/save_descriptions_video/', 'btn-save-files', null, save_textareas)">
...
<script type="text/javascript">
	...
	$('#form-video').on('submit', function(event) {
		...
		var $form = $(this);
		$.post(_base_url + 'cms/article/upload_video/<?php echo $id_article ?>', $(this).serialize(), function(data) {
			hide_loading();
    ...
    $('#buttons-video .delete').on('click', function() {
    	if (confirm('Delete!')) {
    		...
    		$.get(_base_url + 'cms/article/delete_video/' + $this.data('role'), function(data) {
    			hide_loading();
    ...
    $buttons_portada.on('click', function() {
		...
		$.get(_base_url + 'cms/article/set_primary_video/<?php echo $id_article ?>/' + $this.data('role'), function(data) {
			hide_loading();
