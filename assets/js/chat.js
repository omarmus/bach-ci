...
function init_chat() {
    ...
    $('#button-chat, #chat .chat-header').on('click', function (event) {
        event.preventDefault();
        $('#button-chat').toggleClass('active');
        $chat.height( $chat.height() == 0 ? ($('body').height()/2 - 25) : 0 );
    })
...
function set_events_chat (id) {
    ...
    $('#send-message-'+id).on('keyup', function (e) {
        if (e.keyCode == 13 && this.value != null && this.value.length && !/^\s+$/.test(this.value)) {
            ...
            $.post(_base_url + 'admin/chat/save', {id_receiver: _user_current, message: message});
            
            create_message(_user_current, _my_user, message, date);

            // Sending message in chat server
            Chat.sendMessageChat(_user_current, message, date);
